...
import { createSearchArtistPayload } from '../../helpers/index.js'

export class SearchArtistsUseCase implements IUseCase<SearchArgs, z.infer<typeof SearchArtistModel>> {
  constructor() {}

  async execute({ query, limit = 10 }: SearchArgs): Promise<z.infer<typeof SearchArtistModel>> {
    try {
      const res = await useFetch<any>({
        path: endpoints.search,
        method: 'POST',
        body: { query, types: ['AR'], listener: null, count: limit, annotate: true, annotationRecipe: 'CLASS_OF_2019' }
      })
      ...
      if (results.length === 0) {
        throw new HTTPException(404, {
          message: 'No artists could be retrieved for your search.'
        })
      }

      return createSearchArtistPayload(results, limit)
    } catch (error) {
      throw error
    }
  }
